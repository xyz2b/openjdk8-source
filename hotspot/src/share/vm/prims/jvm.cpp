...
JVM_ENTRY(void, JVM_Hello(JNIEnv* env, jclass cls, jobject obj))
    JVMWrapper("JVM_Hello");
  
    printf("=>%s\n", JNIHandles::resolve_non_null(obj)->klass()->name()->as_C_string());
JVM_END

JVM_ENTRY(jint, JVM_GetObjectFieldIntVal(JNIEnv* env, jclass cls, jobject obj, jint offset))
    JVMWrapper("JVM_GetObjectFieldIntVal");
  
    oop r = JNIHandles::resolve_non_null(obj);
    
    //=====    
//    Thread* thread = Thread::current();
    
    printf("%d\n", JNIHandles::get_current_thread_active_handles()->length());
    
    //=====
    return *(int *)((long)r + offset);
JVM_END            
...
JVM_ENTRY(jstring, JVM_GetClassName1(JNIEnv *env, jclass cls))
    assert (cls != NULL, "illegal class");
    JVMWrapper("JVM_GetClassName1");
    JvmtiVMObjectAllocEventCollector oam;
    ResourceMark rm(THREAD);
    const char* name;
    if (java_lang_Class::is_primitive(JNIHandles::resolve(cls))) {
...
JVM_QUICK_ENTRY(jboolean, JVM_IsArrayClass2(JNIEnv *env, jclass cls))
    JVMWrapper("JVM_IsArrayClass2");
    printf("JVM_IsArrayClass2\n");
    
    oop tmp = JNIHandles::resolve_non_null(cls);
    
    Klass* k = java_lang_Class::as_Klass(tmp);
    
    printf("sizeof(oopDesc) = %ld\n", sizeof(oopDesc));
    printf("sizeof(markOopDesc) = %ld\n", sizeof(markOopDesc));
    
//    printf("sizeof(markOopDesc._mark) = %ld\n", sizeof(oopDesc._mark));
//    printf("sizeof(markOopDesc._metadata) = %ld\n", sizeof(oopDesc._metadata));
//    printf("sizeof(markOopDesc._bs) = %ld\n", sizeof(oopDesc._bs));
    
    return true;
JVM_END
            
JVM_ENTRY(void, JVM_ShowInfos(JNIEnv* env, jobject handle))
  JVMWrapper("JVM_ShowInfos");
  printf("===>JVM_ShowInfos");
JVM_END
